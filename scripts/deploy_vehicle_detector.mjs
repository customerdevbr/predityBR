 ...
 *
 * Requer variáveis de ambiente: VPS_HOST, VPS_PASSWORD
 * (defina no .env.local ou exporte no terminal)
 */
...
conn.on('ready', async () => {
    ...
    try {
        ...
        const pkgJson = Buffer.from(JSON.stringify({
            ...
            version: '2.1.0',
            description: 'Backend com detecção IA server-side (otimizado)',
            main: 'server.js',
        ...
        // 3. npm install (onnxruntime-node compila bindings nativos)
        console.log('--- npm install (pode demorar ~2-3 min) ---');
        await execCmd(conn, `${NVM_INIT} && cd ${DIR} && npm install --production 2>&1`);
        ...
        // 5. Reiniciar PM2 com memory limit
        console.log('--- Reiniciando PM2 ---');
        await execCmd(conn, `${NVM_INIT} && pm2 delete contador-veiculos 2>/dev/null; echo "ok"`);
        await execCmd(conn, `${NVM_INIT} && cd ${DIR} && pm2 start server.js --name contador-veiculos --node-args="--max-old-space-size=256"`);
